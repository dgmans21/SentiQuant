"""비-naver 스크래핑 결과(good==True)를 제목+본문 전체로 Qwen 재라벨링.
relabel_full_body.py와 동일한 체크포인트/재시작 패턴.
"""
import os
import re
import sys
...
IN_PATH = "data/raw/nonnaver_scraped.csv"
OUT_PATH = "data/raw/nonnaver_relabeled.csv"
...
if __name__ == "__main__":
    ...
    df = df[df["good"] == True].reset_index(drop=True)
    print(f"good rows total: {len(df)}")
